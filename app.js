...
import { getFirestore, doc, getDoc, setDoc, collection, addDoc, getDocs, query, orderBy, deleteDoc, serverTimestamp }
...
// ── COMMUNITY POSTS ──
async function loadPosts(targetId = 'posts-feed') {
  ...
  try {
    ...
    if (snap.empty) {
      container.innerHTML = '<p style="color:var(--text-muted);text-align:center;padding:2rem 0;">No posts yet. Check back soon!</p>';
      return;
    }

    if (targetId === 'posts-feed') {
      container.innerHTML = snap.docs.map(d => {
        ...
        return `
          <div class="post-card">
            <div class="post-tag">${p.tag || '📢 Announcement'}</div>
            <h3 class="post-title">${p.title}</h3>
            <p class="post-body">${p.body.replace(/\n/g, '<br>')}</p>
            <div class="post-meta">
              <span class="post-author">— ${p.author || 'Synn Lab Staff'}</span>
        ...
        return `
          <div class="admin-post-item">
            <div style="flex:1;min-width:0;">
              <div style="font-size:0.75rem;color:var(--gold);font-weight:600;letter-spacing:0.06em;margin-bottom:0.2rem;">${p.tag}</div>
              <div style="font-family:'Playfair Display',serif;color:var(--green);font-size:1rem;margin-bottom:0.2rem;">${p.title}</div>
              <div style="font-size:0.82rem;color:var(--text-muted);">${p.author} · ${dateStr}</div>
            </div>
            <button onclick="deletePost('${d.id}')"
              style="background:none;border:1.5px solid #e0c0c0;color:#c0392b;padding:0.35rem 0.85rem;border-radius:8px;font-family:'DM Sans',sans-serif;font-size:0.82rem;cursor:pointer;flex-shrink:0;transition:all 0.2s;"
              onmouseover="this.style.background='#fdf0f0'" onmouseout="this.style.background='none'">Delete</button>
          </div>`;
      }).join('');
    }
  } catch(e) {
    console.warn('Posts load error:', e);
    container.innerHTML = '<p style="color:var(--text-muted);text-align:center;padding:2rem 0;">Unable to load posts.</p>';
  }
}
...
window.publishPost = async function() {
  ...
  try {
    await addDoc(collection(db, 'communityPosts'), {
      title, body, author: author || 'Synn Lab Staff', tag, timestamp: serverTimestamp()
    });
    document.getElementById('post-title').value = '';
    document.getElementById('post-body').value = '';
    document.getElementById('post-author').value = '';
    if (msg) { msg.style.display='block'; msg.textContent='✓ Post published!'; setTimeout(()=>{ msg.style.display='none'; }, 3000); }
    loadPosts('admin-posts-list');
  } catch(e) {
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Load posts for public feed
  await loadPosts('posts-feed');
